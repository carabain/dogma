...
/**
 * Unittests for {@link PersonalOracle}.
 */
public class PersonalOracleTest {
    ...
    final Logger logger = LoggerFactory.getLogger(PersonalOracleTest.class);

    private AdminWeb3j web3j = new AdminWeb3j();

    @Test
    public void test() {
        PersonalOracle oracle = new PersonalOracle();

        unlockAccount();
        PersonalChild childContract = deployContract(web3j);

        // Load contract and register event subscriptions
        Personal baseContract = oracle.loadContract(childContract.getContractAddress(), ACCOUNT_ADDRESS);
        oracle.subscribeToEvents(baseContract);

        // Trigger an event by calling the contract
        try {
            logger.info("Triggering event 'aliveRequested' on contract {}", childContract.getContractAddress());
            childContract.isAlive().send();
        } catch (Exception e) {
    ...
    private PersonalChild deployContract(Web3j web3j) {
        PersonalChild contract;
        TransactionManager manager = new ClientTransactionManager(web3j, ACCOUNT_ADDRESS);
        try {
            contract = PersonalChild.deploy(web3j, manager, Contract.GAS_PRICE, Contract.GAS_LIMIT).send();
        } catch (Exception e) {
